...
import hust.team7.filter.util.PixelUtils;
...
public class Gradient extends ArrayColormap implements Cloneable {
	...
	private byte[] knotTypes = { RGB | SPLINE, RGB | SPLINE, RGB | SPLINE,
			RGB | SPLINE };
	...
	private void rebuildGradient() {
		...
		for (int i = 1; i < numKnots - 1; i++) {
			...
			for (int j = xKnots[i]; j < end; j++) {
				...
				Color.RGBToHSV((rgb1 >> 16) & 0xff, (rgb1 >> 8) & 0xff,
						rgb1 & 0xff, hsb1);
				float hsb2[] = new float[3];
				Color.RGBToHSV((rgb2 >> 16) & 0xff, (rgb2 >> 8) & 0xff,
						rgb2 & 0xff, hsb2);
				...
				if (j >= 0 && j <= 255) {
					...
					case LINEAR:
						break;
					case SPLINE:
						// map[i] = ImageMath.colorSpline(j, numKnots, xKnots,
						// yKnots);
					...
					case HUE_CCW:
						...
						float[] hsv = {
								ImageMath.lerp(t, hsb1[0], hsb2[0])
										% (ImageMath.TWO_PI),
								ImageMath.lerp(t, hsb1[1], hsb2[1]),
								ImageMath.lerp(t, hsb1[2], hsb2[2]) };
						/*
						 * float h = ImageMath.lerp(t, hsb1[0], hsb2[0]) %
						 * (ImageMath.TWO_PI); float s = ImageMath.lerp(t,
						 * hsb1[1], hsb2[1]); float b = ImageMath.lerp(t,
						 * hsb1[2], hsb2[2]);
						 */
						// map[j] = 0xff000000 | Color.HSBtoRGB((float)h,
						// (float)s, (float)b);//FIXME-alpha
	...
	public void mutate(float amount) {
		for (int i = 0; i < numKnots; i++) {
			...
			int g = ((rgb >> 8) & 0xff);
			int b = (rgb & 0xff);
			r = PixelUtils.clamp((int) (r + amount * 255
					* (Math.random() - 0.5)));
			g = PixelUtils.clamp((int) (g + amount * 255
					* (Math.random() - 0.5)));
			b = PixelUtils.clamp((int) (b + amount * 255
					* (Math.random() - 0.5)));
