...
public class FieldWarpFilter extends TransformFilter {
	...
	protected void transformInverse(int x, int y, float[] out) {
		...
		for (int line = 0; line < inLines.length; line++) {
			...
			weight = (float) Math
					.pow(Math.pow(l.length, p) / (a + distance), b);
	...
	public int[] filter(int[] src, int w, int h) {
		if (inLines != null && outLines != null) {
			...
			for (int line = 0; line < inLines.length; line++) {
				Line l = intermediateLines[line] = new Line(ImageMath.lerp(
						amount, inLines[line].x1, outLines[line].x1),
						ImageMath.lerp(amount, inLines[line].y1,
								outLines[line].y1), ImageMath.lerp(amount,
								inLines[line].x2, outLines[line].x2),
