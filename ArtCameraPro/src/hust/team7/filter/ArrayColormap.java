...
public class ArrayColormap implements Colormap, Cloneable {
	...
	public int getColor(float v) {
		/*
		 * v *= 255; int n = (int)v; float f = v-n; if (n < 0) return map[0];
		 * else if (n >= 255) return map[255]; return ImageMath.mixColors(f,
		 * map[n], map[n+1]);
		 */
		int n = (int) (v * 255);
		if (n < 0)
	 ...
	 * entry. firstIndex < index < lastIndex must hold.
	 * 
	 * @param index
	 *            the position to set
	 * @param firstIndex
	...
	public void setColorInterpolated(int index, int firstIndex, int lastIndex,
		...
		for (int i = firstIndex; i <= index; i++)
			map[i] = ImageMath.mixColors((float) (i - firstIndex)
					/ (index - firstIndex), firstColor, color);
		for (int i = index; i < lastIndex; i++)
			map[i] = ImageMath.mixColors((float) (i - index)
					/ (lastIndex - index), color, lastColor);
	}
	...
	public void setColorRange(int firstIndex, int lastIndex, int color1,
		...
		for (int i = firstIndex; i <= lastIndex; i++)
			map[i] = ImageMath.mixColors((float) (i - firstIndex)
					/ (lastIndex - firstIndex), color1, color2);
	}
