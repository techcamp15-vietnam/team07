...
public class MotionBlurOp {
	...
	public int[] filter(int[] src, int w, int h) {
		...
		Bitmap tSrcBitmap = Bitmap.createBitmap(src, w, h,
				Bitmap.Config.ARGB_8888);
		...
		for (int i = 0; i < steps; i++) {
			...
			// The .0001 works round a bug on Windows where drawImage throws an
			// ArrayIndexOutofBoundException
			c.scale((float) (1.0001 + scale), (float) (1.0001 + scale), 0.5f,
					0.5f);
